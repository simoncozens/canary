...
Email::Store::Summary->utf8_columns(qw(subject original));
...
sub spec_kinosearch_fields_order { 2 }
sub spec_kinosearch_fields {
    my ($self, $indexer) = @_;
    $indexer->spec_field(name => "subject", stored => 1);
}

sub kinosearch_index {
    my ($self, $mail, $doc) = @_;
    $doc->set_value(subject => $mail->subject);
}
